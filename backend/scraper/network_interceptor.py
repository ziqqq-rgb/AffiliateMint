"""
backend/scraper/network_interceptor.py

Live CDP network wiretap: attaches to a running browser and records
every request/response, with an optional keyword filter. Split out of
scraper/intercept.py so that file can stay pure/tested (see
scraper/intercept.py's parse_response, covered by tests/test_scraper.py)
while this stateful, I/O-heavy class lives on its own.

NOTE: scraper/run.py's harvest loop currently does its own lighter-weight
response handling (scraper/wiretap.py) rather than using this class - if
that stays true, this file may be safe to retire. Confirm nothing else
